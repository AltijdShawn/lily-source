...
// ! NEW COMMAND OBJECTS
// readdirSync
const new_command_folders = readdirSync("src/DGB/newCommands");
for (const folder of new_command_folders) {
  console.log(folder)
  if (folder.endsWith("-example.ts") || folder.endsWith("-example.js")) {}
  else if (folder.endsWith(".ts") || folder.endsWith(".js")) registerNew(folder, "newCommands")
  else {
    const new_command_folders = readdirSync("src/DGB/newCommands/"+folder);
    for (const folder2 of new_command_folders) {
      if (folder2.endsWith("-example.ts") || folder2.endsWith("-example.js")) {}
      else if (folder2.endsWith(".ts") || folder2.endsWith(".js")) registerNew(folder2, "newCommands/"+folder)
      console.log(folder+"/"+folder2)
...
async function registerNew(file, path) {
  const fileName = file.split(".")[0];
  const command = new (await import(`./${path}/${fileName}`)).default;
  // console.log(command)
  console.log(
    `+ [Command handler] Attempting to load: ${file} (main cmd name: ${command.name})`
  );
